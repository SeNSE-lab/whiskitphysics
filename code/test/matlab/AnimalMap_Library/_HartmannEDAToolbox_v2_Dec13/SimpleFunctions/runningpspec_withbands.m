function[t,f,x] = runningpspec (vec1, winsize, shiftwin, SR, bands);
...
while i<=length(vec1)-winsize,
    ...
    for j = 1:nbands
        lo = bands(j,1); hi = bands(j,2);
        a = findc(f,lo,'quiet');
        b = findc(f,hi,'quiet');
        x(counter,j) = sum(dum(a:b));
    end;
