...
class	btIDebugDraw
{
	...
	virtual void drawTruncatedCone(btScalar radius1, btScalar radius2, btScalar height, int upAxis, const btTransform& transform, const btVector3& color)
	{
		int stepDegrees = 30;
		btVector3 start = transform.getOrigin();
		...
		btVector3 capEnd1(0.f,0.f,0.f);
		capEnd1[upAxis] = -halfHeight;
		btVector3 capEnd2(0.f,0.f,0.f);
		capEnd2[upAxis] = halfHeight;

		for (int i=0;i<360;i+=stepDegrees)
		{
			capEnd1[(upAxis+1)%3] = btSin(btScalar(i)*SIMD_RADS_PER_DEG)*radius1;
			capEnd1[(upAxis+2)%3] = btCos(btScalar(i)*SIMD_RADS_PER_DEG)*radius1;
			capEnd2[(upAxis+1)%3] = btSin(btScalar(i)*SIMD_RADS_PER_DEG)*radius2;
			capEnd2[(upAxis+2)%3] = btCos(btScalar(i)*SIMD_RADS_PER_DEG)*radius2;
			drawLine(start+transform.getBasis() * capEnd1,start+transform.getBasis() * capEnd2, color);
		}

		btVector3	offsetRadius1(0,0,0);
		offsetRadius1[(upAxis+1)%3] = radius1;
		btVector3	offset2Radius1(0,0,0);
		offset2Radius1[(upAxis+2)%3] = radius1;
		btVector3	offsetRadius2(0,0,0);
		offsetRadius2[(upAxis+1)%3] = radius2;
		btVector3	offset2Radius2(0,0,0);
		offset2Radius2[(upAxis+2)%3] = radius2;

		drawLine(start+transform.getBasis() * (-offsetHeight+offsetRadius1),start+transform.getBasis() * (offsetHeight+offsetRadius2),color);
		drawLine(start+transform.getBasis() * (-offsetHeight-offsetRadius1),start+transform.getBasis() * (offsetHeight-offsetRadius2),color);
		drawLine(start+transform.getBasis() * (-offsetHeight+offset2Radius1),start+transform.getBasis() * (offsetHeight+offset2Radius2),color);
		drawLine(start+transform.getBasis() * (-offsetHeight-offset2Radius1),start+transform.getBasis() * (offsetHeight-offset2Radius2),color);
		...
		btVector3 xaxis(0,0,0);
		xaxis[(upAxis+1)%3] = btScalar(1.0);
		drawArc(start-transform.getBasis()*(offsetHeight),transform.getBasis()*yaxis,transform.getBasis()*xaxis,radius1,radius1,0,SIMD_2_PI,color,false,10.0);

		// Drawing the top of the cone
		drawArc(start+transform.getBasis()*(offsetHeight),transform.getBasis()*yaxis,transform.getBasis()*xaxis,radius2,radius2,0,SIMD_2_PI,color,false,10.0);
	}
