...
void btCollisionWorld::debugDrawObject(const btTransform& worldTransform, const btCollisionShape* shape, const btVector3& color)
{
	...
	} else
	{
		...
		case TRUNCATED_CONE_SHAPE_PROXYTYPE:
            {
                const btTruncatedConeShape* coneShape = static_cast<const btTruncatedConeShape*>(shape);
                btScalar radius1 = coneShape->getRadius1();//+coneShape->getMargin();
				btScalar radius2 = coneShape->getRadius2();
                btScalar height = coneShape->getHeight();//+coneShape->getMargin();

                int upAxis= coneShape->getConeUpIndex();
                getDebugDrawer()->drawTruncatedCone(radius1, radius2, height, upAxis, worldTransform, color);
                break;
