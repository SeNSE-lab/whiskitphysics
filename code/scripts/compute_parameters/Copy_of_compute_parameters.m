...
ax = get_angles(S,A,num_links);
param.ax = ax;

% group data_avg
geom = [S A];
bp_coor = [x_bp y_bp z_bp];
bp_angles = [EulerThetaRest EulerPhiRest EulerZetaRest]*pi/180;
wpos = [sideID rowID colID];

% save to file
if(~strcmp(pathout,''))
    csvwrite([pathout 'param_name.csv'],Labels)
...
function [x,y,z] = xyz_whisker(S,A,B,num_links)
