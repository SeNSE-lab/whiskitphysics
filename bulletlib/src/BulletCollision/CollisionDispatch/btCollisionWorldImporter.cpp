...
btCollisionShape* btCollisionWorldImporter::convertCollisionShape(  btCollisionShapeData* shapeData  )
{
	...
	//so deal with this
		...
		case TRUNCATED_CONE_SHAPE_PROXYTYPE:
		...
		case CONVEX_HULL_SHAPE_PROXYTYPE:
			{
				...
				switch (shapeData->m_shapeType)
				{
					...
					case TRUNCATED_CONE_SHAPE_PROXYTYPE:
						{
							btTruncatedConeShapeData* conData = (btTruncatedConeShapeData*) shapeData;
							btVector3 dimensions = implicitShapeDimensions;//+margin;
							switch (conData->m_upIndex)
							{
							case 0:
								{
									shape = createTruncatedConeShapeX(dimensions.getY(),dimensions.getZ(), dimensions.getZ());
									break;
								}
							case 1:
								{
									shape = createTruncatedConeShapeY(dimensions.getX(),dimensions.getY(), dimensions.getZ());
									break;
								}
							case 2:
								{
									shape = createTruncatedConeShapeZ(dimensions.getX(),dimensions.getZ(),dimensions.getY());
									break;
								}
							default:
								{
...
btCollisionShape* btCollisionWorldImporter::createTruncatedConeShapeX(btScalar radius1, btScalar radius2, btScalar height)
{
	btTruncatedConeShape* shape = new btTruncatedConeShape(radius1, radius2, height,0);
	m_allocatedCollisionShapes.push_back(shape);
	return shape;
}

btCollisionShape* btCollisionWorldImporter::createTruncatedConeShapeY(btScalar radius1, btScalar radius2, btScalar height)
{
	btTruncatedConeShape* shape = new btTruncatedConeShape(radius1, radius2, height,1);
	m_allocatedCollisionShapes.push_back(shape);
	return shape;
}

btCollisionShape* btCollisionWorldImporter::createTruncatedConeShapeZ(btScalar radius1, btScalar radius2, btScalar height)
{
	btTruncatedConeShape* shape = new btTruncatedConeShape(radius1, radius2, height,2);
	m_allocatedCollisionShapes.push_back(shape);
	return shape;
}
